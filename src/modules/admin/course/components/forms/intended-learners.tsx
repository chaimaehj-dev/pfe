...
const CourseIntendedLearnersForm: FC<IntendedLearnersProps> = ({
  data,
  courseId,
}) => {
  const router = useRouter();
  const [intendedLearners, setIntendedLearners] = useState<string[]>(data);
  ...
  useEffect(() => {
    const isDataChanged = !arraysAreEqual(intendedLearners, data);
    setHasChanged(isDataChanged);
  }, [intendedLearners, data]);
  ...
  const handleSave = async () => {
    try {
      ...
      const cleanedData = intendedLearners.filter((item) => item.trim() !== "");
        ...
        <div>
          <div className="border-stroke dark:border-dark-3 border-b">
            ...
            <p className="text-body-color dark:text-dark-6 mb-6 text-sm font-medium">
              Use this section to describe the ideal learners for your course.
              Think about their goals, challenges, background knowledge, or
              professional context. Be as clear as possible so learners know if
              the course fits their needs.
            </p>
          </div>
        </div>
      </div>
...
export default CourseIntendedLearnersForm;
