...
const CoursePrerequisitesForm: FC<PrerequisitesProps> = ({
  data,
  courseId,
}) => {
  const router = useRouter();
  const [prerequisites, setPrerequisites] = useState<string[]>(data);
  ...
  useEffect(() => {
    const isDataChanged = !arraysAreEqual(prerequisites, data);
  ...
  const handleSave = async () => {
    try {
      ...
      const cleanedData = prerequisites.filter((item) => item.trim() !== "");
        ...
        <div>
          <div className="border-stroke dark:border-dark-3 border-b">
            ...
            <p className="text-body-color dark:text-dark-6 mb-6 text-sm font-medium">
              Use this section to list the knowledge, skills, or tools learners
              should have before starting this course. Setting clear
              prerequisites helps ensure students are prepared and can fully
